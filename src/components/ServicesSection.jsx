                        ...
                        services.map((service, index) => {
                            return <div key={index} className='bg-white max-w-72 cursor-pointer rounded-2xl p-6 hover:shadow-xl hover:scale-105 transition-all duration-300'>
                                <motion.div
                                    ...
                                    initial="hidden"
                                    whileInView="show"
                                    className='mb-2 '>{service.icon}</motion.div>
                                <motion.h3
                                    variants={fadeIn("up", 0.3)}
                                    initial="hidden"
                                    whileInView="show"
                                    className='text-xl font-semibold'>{service.title}</motion.h3>
                                <motion.p
                                    variants={fadeIn("up", 0.4)}
                                    initial="hidden"
                                    whileInView="show"
                                    className='text-gray-600 mb-1'>{service.description}</motion.p>
                                <motion.a
                                    variants={fadeIn("up", 0.5)}
                                    initial="hidden"
                                    whileInView="show"
                                    href={service.link} className='text-indigo-500 font-medium hover:text-indigo-700 transition-colors'>LEARN MORE</motion.a>
                            </div>
                        })
                    }
